...
for split in "${splits[@]}"; do
    ...
    for model in "${models[@]}"; do
        ...
        # CUDA_VISIBLE_DEVICES=0,1 accelerate launch --config_file configs/accelerate/default_config.yaml --main_process_port $MASTER_PORT \
        # src/train.py experiment=finetune/tofu/default.yaml \
        # task_name=tofu_${model}_special_retain1 \
        # model=${model} \
        # data/datasets@data.train=TOFU_QA_retain \
        # data.train.TOFU_QA_retain.args.hf_args.name=${retain_split} \
        ...
        # CUDA_VISIBLE_DEVICES=0,1 accelerate launch --config_file configs/accelerate/default_config.yaml --main_process_port $MASTER_PORT \
        # src/train.py experiment=finetune/tofu/default.yaml \
        # task_name=tofu_${model}_special_forget1 \
        # model=${model} \
        # data/datasets@data.train=TOFU_QA_forget \
        # data.train.TOFU_QA_forget.args.hf_args.name=${forget_split} \
        # model.model_args.pretrained_model_name_or_path=saves/finetune/tofu_${model}_special_retain1 \
        # trainer.args.per_device_train_batch_size=4 \
        # trainer.args.ddp_find_unused_parameters=true \
        # trainer.args.gradient_checkpointing=true \
        # trainer.args.warmup_epochs=0 \
        ...
        for epoch in {2..5}; do
            ...
            CUDA_VISIBLE_DEVICES=0,1 accelerate launch --config_file configs/accelerate/default_config.yaml --main_process_port $MASTER_PORT \
            src/train.py experiment=finetune/tofu/default.yaml \
            task_name=tofu_${model}_special_retain${epoch} \
            model=${model} \
            data/datasets@data.train=TOFU_QA_retain \
            data.train.TOFU_QA_retain.args.hf_args.name=${retain_split} \
            model.model_args.pretrained_model_name_or_path=saves/finetune/tofu_${model}_special_forget$((epoch - 1)) \
            trainer.args.per_device_train_batch_size=4 \
            trainer.args.ddp_find_unused_parameters=true \
            trainer.args.gradient_checkpointing=true \
            trainer.args.warmup_epochs=0 \
            ...
            CUDA_VISIBLE_DEVICES=0,1 accelerate launch --config_file configs/accelerate/default_config.yaml --main_process_port $MASTER_PORT \
            src/train.py experiment=finetune/tofu/default.yaml \
            task_name=tofu_${model}_special_forget${epoch} \
            model=${model} \
            data/datasets@data.train=TOFU_QA_forget \
            data.train.TOFU_QA_forget.args.hf_args.name=${forget_split} \
            model.model_args.pretrained_model_name_or_path=saves/finetune/tofu_${model}_special_retain${epoch} \
            trainer.args.per_device_train_batch_size=4 \
            trainer.args.ddp_find_unused_parameters=true \
            trainer.args.gradient_checkpointing=true \
            trainer.args.warmup_epochs=0 \
