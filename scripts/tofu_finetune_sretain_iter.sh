...
for split in "${splits[@]}"; do
    ...
    trim_count=$(( tofu_trim_base * (100 - retain_pct) / 100 ) // 2)
    ...
    for model in "${models[@]}"; do
        ...
        task_name=tofu_${model}_${retain_split}_mimic_iter1_epoch${epoch} \
        model=${model} \
        ...
        CUDA_VISIBLE_DEVICES=0,1 accelerate launch --config_file configs/accelerate/default_config.yaml --main_process_port $MASTER_PORT \
        src/train.py experiment=finetune/tofu/default.yaml \
        task_name=tofu_${model}_${retain_split}_mimic_iter2_epoch${epoch} \
        model=${model} \
        data/datasets@data.train=TOFU_QA_retain \
        data.train.TOFU_QA_retain.args.hf_args.name=${retain_split} \
        data.train.TOFU_QA_retain.args.hf_args.split="train\[${trim_count}:${((2*trim_count))}\]" \
        model.model_args.pretrained_model_name_or_path=/cnz/data/ms-home/Llama-2-7b-chat-hf \
        trainer.args.learning_rate=1e-5 \
        trainer.args.per_device_train_batch_size=${per_device_train_batch_size} \
        trainer.args.gradient_accumulation_steps=2 \
