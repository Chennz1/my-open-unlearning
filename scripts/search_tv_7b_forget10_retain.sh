...
forget_path="/cnz/data/project/my-open-unlearning/saves/finetune/tofu_Llama-2-7b-chat-hf_reinforce_forget10_epoch10"
retain_path="/cnz/data/project/my-open-unlearning/saves/finetune/tofu_Llama-2-7b-chat-hf_reinforce_retain90_epoch10"

# Output base directory
output_base="/cnz/data/project/my-open-unlearning/saves/unlearn/tv_reinforce_forget10_comp_Llama-2-7b-chat-hf_mlp"
mkdir -p ${output_base}
...
for alpha in "${alphas[@]}"; do
    ...
    # Create temporary model path with alpha in the name
    temp_model_path="${output_base}/model_a${alpha}"

    # Log current experiment
    echo "[$(date)] Starting alpha=${alpha}" >> ${log_file}

    # Step 1: Merge models with current alpha
    echo "Step 1/3: Merging models..."
    CUDA_VISIBLE_DEVICES=${device} python scripts/effiecient_task_vector_retain.py \
    ...
    if [ $? -ne 0 ]; then
        echo "Error: Evaluation failed for alpha=${alpha}" | tee -a ${log_file}
    else
    ...
    if [ -d "${temp_model_path}" ]; then
        ...
        if [ -d "${temp_model_path}/evals" ]; then
            ...
            mv ${temp_model_path}/evals ${output_base}/evals_a${alpha}
        fi
...
for alpha in "${alphas[@]}"; do
    evals_dir="${output_base}/evals_a${alpha}"
    if [ -d "${evals_dir}" ]; then
        echo "Alpha=${alpha}:" >> ${summary_file}
