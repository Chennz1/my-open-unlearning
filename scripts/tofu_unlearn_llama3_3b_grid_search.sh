...
# # NPO: beta in [0.05, 0.2], gamma in [0.2, 0.5]
# trainer_configs["NPO"]="unlearn/tofu/default.yaml|0.05,0.1,0.15,0.2|0.2,0.3,0.4,0.5"

# # SimNPO: beta in [2.0, 3.0], gamma in [0.2, 0.3]
trainer_configs["SimNPO"]="unlearn/tofu/default.yaml|1.5,2.0,2.5,3.0,3.5|0.05,0.1,0.125,0.15,0.175"

# GradDiff: can add other methods here with their parameter ranges
# trainer_configs["GradDiff"]="unlearn/tofu/default.yaml|1.0|1.0"
        ...
        for trainer in "${!trainer_configs[@]}"; do
            ...
            for beta in "${betas[@]}"; do
                for gamma in "${gammas[@]}"; do
                    ...
                    eval_file="saves/unlearn/${task_name}/evals/TOFU_EVAL.json"
                    if [ -f "$eval_file" ]; then
                        echo "Skipping ${task_name} because evaluation file exists: ${eval_file}"
                        continue
                    fi

                    model_path=/cnz/data/project/my-open-unlearning/saves/finetune/tofu_Llama-3.2-3B-Instruct_full
                    echo ""
