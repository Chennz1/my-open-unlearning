...
forget_path="/cnz/data/project/my-open-unlearning/saves/finetune/tofu_Llama-2-7b-chat-hf_forget05_epoch5"
retain_mimic_path="/cnz/data/project/my-open-unlearning/saves/finetune/tofu_Llama-2-7b-chat-hf_retain95_mimic_epoch5"

# Output base directory
output_base="/cnz/data/project/my-open-unlearning/saves/unlearn/hyperparam_forget05_7b_mlp"
mkdir -p ${output_base}
...
for alpha in "${alphas[@]}"; do
    for beta in "${betas[@]}"; do
        ...
        target_eval_dir="${output_base}/evals_a${alpha}_b${beta}"
        if [ -f "${target_eval_dir}/TOFU_EVAL.json" ]; then
             echo "Skipping alpha=${alpha}, beta=${beta} [${current}/${total_combinations}] - already completed"
             continue
        fi
