...
# trainer_configs["SimNPO"]="unlearn/tofu/default.yaml|2.0,2.25,2.5|0.1,0.15,0.2"

# GradDiff: can add other methods here with their parameter ranges
# trainer_configs["GradDiff"]="unlearn/tofu/default.yaml|0.5,1.0,2.0,5.0,10.0|1.0"
        ...
        for trainer in "${!trainer_configs[@]}"; do
            ...
            for beta in "${betas[@]}"; do
                for gamma in "${gammas[@]}"; do
                    ...
                    eval_dir=saves/unlearn/${task_name}/evals
                    echo ""
                    echo ">>> ${task_name}: Unlearning with ${trainer} (beta=${beta}, gamma=${gamma})"

                    # Skip if evaluation results already exist (completed in a previous run)
                    if [ -d "${eval_dir}" ]; then
                        echo ">>> Skipping ${task_name} (found ${eval_dir})"
                        continue
                    fi
